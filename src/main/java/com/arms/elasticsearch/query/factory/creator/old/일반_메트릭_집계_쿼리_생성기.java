...
import com.arms.elasticsearch.query.base.기본_하위_집계_요청;
...
@Getter
	...
	private 일반_메트릭_집계_쿼리_생성기(기본_하위_집계_요청 기본_하위_집계_요청, EsQuery esQuery){
		this.termsAggregationBuilder
			= AggregationBuilders.terms("group_by_" + 기본_하위_집계_요청.get메인_그룹_필드())
				.field(기본_하위_집계_요청.get메인_그룹_필드());

		this.nativeSearchQueryBuilder.addAggregation(
				this.termsAggregationBuilder
		);

		Optional.of(기본_하위_집계_요청.get하위크기()).filter(a->a>0)
			.ifPresent(a->{
				this.termsAggregationBuilder.size(기본_하위_집계_요청.get하위크기());
			});

		this.메인_그룹_필드 = 기본_하위_집계_요청.get메인_그룹_필드();
		this.그룹_필드들 = 기본_하위_집계_요청.get하위_그룹_필드들();
		this.esQuery = esQuery;
	}

	public static 쿼리_생성기 of(기본_하위_집계_요청 기본_하위_집계_요청, EsQuery esQuery){
		return new 일반_메트릭_집계_쿼리_생성기(기본_하위_집계_요청, esQuery);
	}

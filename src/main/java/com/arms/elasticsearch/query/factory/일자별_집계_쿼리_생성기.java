...
@Setter
@Getter
public class 일자별_집계_쿼리_생성기 implements 쿼리_추상_팩토리 {
	...
	private 일자별_집계_쿼리_생성기(기본_집계_요청 기본_집계_요청, EsQuery esQuery){
	...
	public static 쿼리_추상_팩토리 of(기본_집계_요청 기본_집계_요청, EsQuery esQuery){
		return new 일자별_집계_쿼리_생성기(기본_집계_요청, esQuery);
	}
		...
		Optional.ofNullable(메인그룹필드)
			.ifPresent(시간그룹필드 -> {
				...
				Optional.ofNullable(하위그룹필드들)
					.ifPresent(하위그룹필드들->{
						if(!하위그룹필드들.isEmpty()){
							dateHistogramAggregationBuilder.subAggregation(하위_집계_빌더_생성기.createNestedAggregation(하위그룹필드들, 크기));
						}
					});
			});

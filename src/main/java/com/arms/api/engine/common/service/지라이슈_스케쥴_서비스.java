...
public interface 지라이슈_스케쥴_서비스 {
    ...
    이슈_인덱스 이슈_추가하기(이슈_인덱스 이슈_인덱스);

    int 대량이슈_추가하기(List<이슈_인덱스> 대량이슈_리스트);

    이슈_인덱스 이슈_조회하기(String 조회조건_아이디);

    List<이슈_인덱스> 이슈_검색하기(검색조건 검색조건);

    이슈_인덱스 이슈_검색엔진_저장(Long 지라서버_아이디, String 이슈_키, Long 제품서비스_아이디, Long[] 제품서비스_버전_아이디들, Long cReqLink) throws Exception;

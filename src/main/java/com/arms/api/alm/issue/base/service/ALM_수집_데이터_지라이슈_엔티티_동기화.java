...
public class ALM_수집_데이터_지라이슈_엔티티_동기화 {
    ...
    private final List<지라이슈_엔티티> 지라이슈_엔티티_요구사항_연결이슈_목록;
    ...
    private ALM_수집_데이터_지라이슈_엔티티_동기화(지라이슈_벌크_추가_요청 지라이슈_벌크_추가_요청값,List<지라이슈_데이터> 지라이슈_데이터_하위이슈_목록,List<지라이슈_데이터> 지라이슈_데이터_연결이슈_목록){
        ...
        this.지라이슈_엔티티_요구사항_연결이슈_목록 = getBean(서브테스크_조회.class).요구사항_연결이슈_검색하기(지라이슈_벌크_추가_요청값);
    ...
    public static ALM_수집_데이터_지라이슈_엔티티_동기화  ALM_수집_데이터_지라이슈_엔티티_동기화_생성(지라이슈_벌크_추가_요청 지라이슈_벌크_추가_요청값){
        ...
        return new ALM_수집_데이터_지라이슈_엔티티_동기화(지라이슈_벌크_추가_요청값,getBean(이슈전략_호출.class).서브테스크_가져오기(지라이슈_벌크_추가_요청값),getBean(이슈전략_호출.class).이슈링크_가져오기(지라이슈_벌크_추가_요청값));

    }

    public static ALM_수집_데이터_지라이슈_엔티티_동기화  ALM_수집_데이터_증분_지라이슈_엔티티_동기화_생성(지라이슈_벌크_추가_요청 지라이슈_벌크_추가_요청값){

        return new ALM_수집_데이터_지라이슈_엔티티_동기화(지라이슈_벌크_추가_요청값,getBean(이슈전략_호출.class).증분서브테스크_가져오기(지라이슈_벌크_추가_요청값),getBean(이슈전략_호출.class).증분이슈링크_가져오기(지라이슈_벌크_추가_요청값));
    ...
    public void 지라이슈_엔티티_연결이슈_적용(){
        if (this.지라이슈_데이터_연결이슈_목록 != null && !this.지라이슈_데이터_연결이슈_목록.isEmpty()) {

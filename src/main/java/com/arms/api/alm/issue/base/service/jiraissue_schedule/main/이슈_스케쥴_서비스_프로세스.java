...
public class 이슈_스케쥴_서비스_프로세스 implements 이슈_스케쥴_서비스 {
    ...
    public int 증분이슈_링크드이슈_서브테스크_벌크추가(@Valid 지라이슈_벌크_추가_요청 지라이슈_벌크_추가_요청값) throws Exception {
        ...
        ALM_수집_데이터_증분_지라이슈_엔티티_동기화 ALM_수집_데이터_증분_지라이슈_엔티티_동기화 =  new ALM_수집_데이터_증분_지라이슈_엔티티_동기화(지라이슈_벌크_추가_요청값);

        return 대량이슈_추가하기(ALM_수집_데이터_증분_지라이슈_엔티티_동기화.지라이슈_앤티티_저장할_목록_가져오기());
    }

...
import static com.arms.api.alm.issue.base.service.지라이슈_엔티티_삭제처리.*;
...
public class 이슈_스케쥴_서비스_프로세스 implements 이슈_스케쥴_서비스 {
    ...
    public int 이슈_링크드이슈_서브테스크_벌크로_추가하기(@Valid 지라이슈_벌크_추가_요청 지라이슈_벌크_추가_요청값) throws Exception {
        ...
        지라이슈_엔티티_컬렉션 지라이슈_엔티티_컬렉션값 = new 지라이슈_엔티티_컬렉션(new ArrayList<>());
        ...
        if (반환된_이슈 == null) {
            ...
            지라이슈_엔티티 삭제_지라이슈 = 지라이슈_요구사항_연결_끊고_삭제_적용(this.이슈_조회하기(조회조건_아이디));

            지라이슈_엔티티_컬렉션값.엔티티_목록_추가(삭제_지라이슈);

            // 하위이슈의 경우 요구사항 이슈가 삭제되었을 때 전부 삭제된다(모든 도큐먼트에 삭제 flag 추가)

            List<지라이슈_엔티티> 삭제할_요구사항_서브테스크_목록값  = 서브테스크_조회.요구사항_서브테스크_검색하기(지라서버_아이디, 이슈_키);

            지라이슈_엔티티_컬렉션값.엔티티_목록_리스트_전체_추가(
                지라이슈_일괄_삭제_적용(삭제할_요구사항_서브테스크_목록값)
            );
            ...
            //지라이슈_데이터_컬렉션 이슈링크_데이터_컬렉션값 = new 지라이슈_데이터_컬렉션();

            //List<지라이슈_엔티티> es에_저장된_이슈링크_목록 = null;
            지라이슈_엔티티_컬렉션 es에_저장된_서브테스크_목록 = new 지라이슈_엔티티_컬렉션(서브테스크_조회.요구사항_서브테스크_검색하기(지라서버_아이디,이슈_키));

            지라이슈_데이터_컬렉션 서브테스크이슈_데이터_컬렉션값 = new 지라이슈_데이터_컬렉션(이슈전략_호출.서브테스크_가져오기(지라서버_아이디, 이슈_키));
            //List<지라이슈_데이터> 이슈링크_가져오기 = 이슈전략_호출.이슈링크_가져오기(지라서버_아이디, 이슈_키);

            //이슈링크_데이터_컬렉션값.데이터_목록_리스트_전체_추가(이슈링크_가져오기);
            ...
            if (es에_저장된_서브테스크_목록.isNotEmpty()){
                List<지라이슈_엔티티> 지라이슈_엔티티에_존재하지_않는_목록_찾기 = es에_저장된_서브테스크_목록.지라이슈_엔티티에_존재하지_않는_목록_찾기(서브테스크이슈_데이터_컬렉션값);
                List<지라이슈_엔티티> 지라이슈_일괄_삭제_적용 = 지라이슈_일괄_삭제_적용(지라이슈_엔티티에_존재하지_않는_목록_찾기);
                지라이슈_엔티티_컬렉션값.엔티티_목록_리스트_전체_추가(지라이슈_일괄_삭제_적용);
            }

            if (서브테스크이슈_데이터_컬렉션값.get지라이슈_데이터_목록값() != null && !서브테스크이슈_데이터_컬렉션값.get지라이슈_데이터_목록값().isEmpty()) {
                List<지라이슈_엔티티> 변환된_이슈_목록 = 서브테스크이슈_데이터_컬렉션값.get지라이슈_데이터_목록값().stream().map(ALM서브테스크 -> {
             ...
             * */
           /* if (이슈링크_데이터_컬렉션값.get지라이슈_데이터_목록값() != null &&!서브테스크이슈_데이터_컬렉션값.get지라이슈_데이터_목록값().isEmpty()) {
                List<지라이슈_엔티티> 변환된_이슈링크_목록 = 이슈링크_데이터_컬렉션값.get지라이슈_데이터_목록값().stream().map(ALM이슈링크 -> {

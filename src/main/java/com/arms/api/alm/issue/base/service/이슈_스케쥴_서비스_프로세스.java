...
public class 이슈_스케쥴_서비스_프로세스 implements 이슈_스케쥴_서비스 {
    ...
    public int 이슈_링크드이슈_서브테스크_벌크로_추가하기(@Valid 지라이슈_벌크_추가_요청 지라이슈_벌크_추가_요청값) throws Exception {
        ...
        지라이슈_엔티티_삭제_적용 지라이슈_엔티티_삭제_적용 = new 지라이슈_엔티티_삭제_적용();
        지라이슈_엔티티_컬렉션 지라이슈_엔티티_저장_목록 = new 지라이슈_엔티티_컬렉션(new ArrayList<>());
        ...
        if (반환된_이슈 == null) {
            ...
            지라이슈_엔티티_삭제_적용.지라이슈_요구사항_연결_끊고_삭제_적용(this.이슈_조회하기(조회조건_아이디));

            지라이슈_엔티티_삭제_적용.지라이슈_일괄_삭제_적용(서브테스크_조회.요구사항_서브테스크_검색하기(지라서버_아이디, 이슈_키));

        } else {

            지라이슈_엔티티_저장_목록.엔티티_목록_추가(
                지라이슈_생성.ELK_데이터로_변환(지라서버_아이디, 반환된_이슈, true, null, 제품서비스_아이디, 제품서비스_버전들, cReqLink)
            );

            지라이슈_엔티티_컬렉션 지라이슈_엔티티_컬렉션 = new 지라이슈_엔티티_컬렉션(서브테스크_조회.요구사항_서브테스크_검색하기(지라이슈_벌크_추가_요청값));

            지라이슈_데이터_컬렉션 지라이슈_데이터_컬렉션 = new 지라이슈_데이터_컬렉션(이슈전략_호출.서브테스크_가져오기(지라이슈_벌크_추가_요청값));

            지라이슈_엔티티_삭제_적용.지라이슈_데이터에_존재하지_않는_지라이슈_엔티티_삭제_적용(지라이슈_엔티티_컬렉션,지라이슈_데이터_컬렉션);

            지라이슈_데이터_컬렉션.get지라이슈_데이터_목록()
                .forEach(ALM서브테스크 -> {
                    지라이슈_엔티티 변환된_이슈 =
                        지라이슈_생성.ELK_데이터로_변환(지라서버_아이디, ALM서브테스크,false, 이슈_키, 제품서비스_아이디, 제품서비스_버전들, cReqLink);
                    지라이슈_엔티티_저장_목록.엔티티_목록_추가(변환된_이슈);
                });
        }

        List<지라이슈_엔티티> 삭제할_지라이슈_엔티티_목록 = 지라이슈_엔티티_삭제_적용.삭제할_지라이슈_엔티티_목록();
        List<지라이슈_엔티티> 지라이슈_엔티티_목록 = 지라이슈_엔티티_저장_목록.get지라이슈_엔티티_목록();
        지라이슈_엔티티_목록.addAll(삭제할_지라이슈_엔티티_목록);

        return 대량이슈_추가하기(지라이슈_엔티티_목록);
    }

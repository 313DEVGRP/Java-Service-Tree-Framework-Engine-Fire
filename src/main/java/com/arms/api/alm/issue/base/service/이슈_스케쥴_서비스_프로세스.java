...
public class 이슈_스케쥴_서비스_프로세스 implements 이슈_스케쥴_서비스 {
    ...
    public int 이슈_링크드이슈_서브테스크_벌크로_추가하기(@Valid 지라이슈_벌크_추가_요청 지라이슈_벌크_추가_요청값) throws Exception {
        ...
        ALM_수집_데이터_엔티티_적용 ALM_수집_데이터_엔티티_적용 = new ALM_수집_데이터_엔티티_적용(지라이슈_벌크_추가_요청값);

        지라이슈_엔티티_컬렉션 지라이슈_엔티티_저장_목록 = new 지라이슈_엔티티_컬렉션(new ArrayList<>());

        if (ALM_수집_데이터_엔티티_적용.get이슈_상세정보_가져오기() == null) {

            ALM_수집_데이터_엔티티_적용.지라이슈_요구사항_연결_끊고_삭제_적용(
                this.이슈_조회하기(지라이슈_벌크_추가_요청값.조회조건_아이디())
            );

            ALM_수집_데이터_엔티티_적용.지라이슈_일괄_삭제_적용();

        } else {

            ALM_수집_데이터_엔티티_적용.지라이슈_데이터에_존재하지_않는_지라이슈_엔티티_삭제_적용();

            ALM_수집_데이터_엔티티_적용.지라이슈_엔티티_요구사항_적용();

            ALM_수집_데이터_엔티티_적용.지라이슈_엔티티_하위이슈_적용();
        }


        return 대량이슈_추가하기(지라이슈_엔티티_저장_목록.get지라이슈_엔티티_목록());
    }

...
public class 이슈_스케쥴_서비스_프로세스 implements 이슈_스케쥴_서비스 {
    ...
    public int 이슈_링크드이슈_서브테스크_벌크로_추가하기(@Valid 지라이슈_벌크_추가_요청 지라이슈_벌크_추가_요청값) throws Exception {
        ...
        if (반환된_이슈 == null) {
            ...
            지라이슈_데이터_컬렉션 서브테스크이슈_데이터_컬렉션값 = new 지라이슈_데이터_컬렉션();
            ...
            서브테스크이슈_데이터_컬렉션값.데이터_목록_리스트_전체_추가(서브테스크_가져오기);
            ...
            if (es에_저장된_서브테스크_목록 != null && !es에_저장된_서브테스크_목록.isEmpty()){
                ...
                List<지라이슈_엔티티> 삭제된_서브테스크_목록 = 삭제된_ALM_하위이슈_ES_데이터_반환(서브테스크_가져오기, es에_저장된_서브테스크_목록, 이슈_삭제_년월일);
